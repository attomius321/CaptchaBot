...
const CONFIG = {
  target: {
    ...
    sliderSelector: ".cf-modal__challenge",
...
class ColorDetector {
  ...
  matches(r, g, b) {
    return (
      Math.abs(r - this.target.r) <= this.tolerance &&
      Math.abs(g - this.target.g) <= this.tolerance &&
      Math.abs(b - this.target.b) <= this.tolerance
    );
  }
...
class CaptchaSolver {
  ...
  async solveSliderChallenge() {
    ...
    const found = await this.colorDetector.findPixel(screenshotBuffer);
    ...
    const targetPageX = sliderBox.x + found.x * scaleX;
